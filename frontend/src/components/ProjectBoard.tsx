...
const ProjectBoard = () => {
  ...
  return (
    <div className='relative bw-border w-full h-[80%] p-10 flex flex-col gap-5 overflow-auto'>
      ...
      <p>{`This projest has: ${tasks.length} tasks`}</p>
      ...
      <div className='flex flex-col gap-5'>
        <div className='flex flex-wrap gap-8 justify-evenly'>
            <ProjectBoardColumn heading={"Not started"} tasks={todoTasks} 
              createTask={handleCreateTask} 
              updateTask={handleUpdateTask} 
              deleteTask={handleDeleteTask} />
            <ProjectBoardColumn heading={"In Progress"} tasks={inProgressTasks}
              createTask={handleCreateTask}
              updateTask={handleUpdateTask}
              deleteTask={handleDeleteTask} />
            <ProjectBoardColumn heading={"Completed"} tasks={completedTasks}
              createTask={handleCreateTask}
              updateTask={handleUpdateTask}
              deleteTask={handleDeleteTask} />
        </div>
